...
if($authenticated&&!$error&&$_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($action==='save'&&isset($definitions[$collection])&&$editing){
  ...
  if($collection==='projects'&&!in_array($data['theme']??'',['medical','treeva','education','crm','hrms','warehouse','spiritual'],true))$error='Choose a valid project visual theme.';
  if($collection==='posts'){
